...
# Constants for tool versions (updated as of June 2025)
...
# Installation Commands (Ubuntu/Debian-centric, update as needed for other distros)
declare -A install_commands=(
  ...
  [jenkins]="curl -fsSL $JENKINS_REPO/debian/jenkins.io-2023.key | sudo tee \
    /usr/share/keyrings/jenkins-keyring.asc > /dev/null && echo deb [signed-by=/usr/share/keyrings/jenkins-keyring.asc] $JENKINS_REPO/debian binary/ | sudo tee /etc/apt/sources.list.d/jenkins.list > /dev/null && sudo apt-get update && sudo apt-get install -y fontconfig openjdk-17-jre jenkins"
  ...
  [grafana]="wget $GRAFANA_DEB_URL && sudo dpkg -i $(basename $GRAFANA_DEB_URL) && rm $(basename $GRAFANA_DEB_URL)"
